function onThink(interval)
	if _Lib_Battle_Days[os.date("%A")] then
		...
		if tb and (tb.players % 2 == 0) then
			...
			broadcastMessage("[BattleField] Um teleport foi criado no templo de Thais, apenas " .. tb.players .. " jogadores poderao entrar, para serem divididos em " .. ((tb.players) / 2) .. " VS " .. ((tb.players) / 2) .. ".")
		end
	end
	return true
end
